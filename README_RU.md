...
От вопроса до ответа с гейтом:

1. **Вопрос** — Streamlit отдаёт ответ со ссылками, маршрут (`simple` / `complex`) и
   отрендеренные версии промптов.

   ![Ответ со ссылками](./docs/assets/demo/ui-answer.png)

2. **Q&A подразделений** — лист объекта передаётся типизированным профилем, а не поиском:
   ответ разделяет факты объекта и применённые выводы и остаётся условным, когда нужное
   поле — `unknown`.
